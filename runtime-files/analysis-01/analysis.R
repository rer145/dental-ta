...
do.get.age<-function (src='',id=1,lo=0.75,hi=25.75,left=0,right=25,def.int=0.1,
          ...
          teeths=rep(NA,13),lab=NA,anote='')
{
  ...
  ##################################################################
  if(!is.na(id)) teeth=as.numeric(src[id,-1])
  else(teeth=teeths)
  if(nchar(anote)>0) anote=paste(', ',anote)
  #write('Tooth scores before any potential dropped scores', file=output_file, append=FALSE, sep='')
  ...
  if(!is.na(id)){
    plot(exp(age)-.75,prob,type='l',xlim=c(left,right),ylim=c(0,top),lwd=2,
         xlab='Age (years)',ylab='Density',main=paste(src[id,1],anote),axes=F)}
  else
  {
    plot(exp(age)-.75,prob,type='l',xlim=c(left,right),ylim=c(0,top),lwd=2,
  ...
  if(!is.na(id)) (lab=as.vector(src[id,1]))
  new.teeth=rep(NA,13)
  for(i in 1:13){
  ...
  output_text<-gsub("[[LIKELY_SCORES]]", paste('[', gsub("NA", "-1", paste(new.teeth, collapse=',')), ']', sep=''), output_text, fixed=TRUE)

  if (is.na(between)) 
  	output_text<-gsub("[[P_SEQ]]", "'NA'", output_text, fixed=TRUE)
  else 
  	output_text<-gsub("[[P_SEQ]]", obj/nu, output_text, fixed=TRUE)
...
do.plot.teeth<-function(src,id=1,Known.L=-10,Known.R=-10,teeths=c(NA,15,14,NA,5,NA,NA,9,6,5,NA,NA,NA),
...
temp <- read.csv(input_file)
do.get.age(temp, row_num)
do.plot.teeth(temp, row_num)